...
#from fcvore.nn import FlopCountAnalysis
def calc_FLOPs(model, batch_size):
    model.eval()
    input_size = model.layer1()
    test_sample = torch.random((batch_size, input_size))
    flops = FlopCountAnalysis(model, [test_sample])
    return flops.total()
